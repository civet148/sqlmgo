...
func (r *Result) formatSqlNode() (*Result, error) {
	...
	_ = sqlparser.Walk(func(node sqlparser.SQLNode) (kontinue bool, err error) {
		...
		case sqlparser.GroupBy:
			...
			if len(groupby) != 0 {
				r.MgoType = MgoType_GroupBy
			}
		case *sqlparser.Insert:
			r.MgoType = MgoType_Insert
		case *sqlparser.Delete:
			r.MgoType = MgoType_Delete
		case *sqlparser.Update:
			r.MgoType = MgoType_Update
		case *sqlparser.Select:
			r.MgoType = MgoType_Query
		}
		return true, nil
	}, r.Stmt)
	log.Debugf("mongo type [%v]", r.MgoType.String())
	_ = buf
